...
namespace TownOfUs.SwapperMod
{
    ...
    public class SwapVotes
    {
        ...
        private static void SwapPositions(Transform a, Transform b, float duration = 2.5f)
        {
            Coroutines.Start(Slide2D(a, b.position, duration));
            Coroutines.Start(Slide2D(b, a.position, duration));
        }

        private static IEnumerator Slide2D(Transform target, Vector3 dest, float duration = 2.5f)
        {
