...
namespace TownOfUs
{
    public static class RpcHandling
    {
        ...
        private static void GenEachRole(List<GameData.PlayerInfo> infected)
        {
            ...
            canHaveModifier.RemoveSingle(player => player.Is(RoleEnum.Altruist));
            ...
            foreach (var (type, rpc, _) in GlobalModifiers)
            {
                ...
                Role.Gen<Modifier>(
                    ...
                    canHaveModifier,
                    rpc,
                    rpc == CustomRPC.SetSnitch && CustomGameOptions.SnitchTargetsCrewmate
                );
            }

            canHaveModifier.RemoveAll(player => Role.GetRole(player)?.Faction != Faction.Crewmates);
            canHaveModifier.Shuffle();

            while (canHaveModifier.Count > 0 && CrewmateModifiers.Count > 0)
            {
                var (type, rpc, _) = CrewmateModifiers.TakeFirst();
                Role.Gen<Modifier>(type, canHaveModifier, rpc, rpc == CustomRPC.SetSnitch);
            }
                ...
                switch ((CustomRPC) callId)
                {
                    ...
                    case CustomRPC.JanitorClean:
                        ...
                        foreach (var body in deadBodies)
                            ...
                            {
                                Coroutines.Start(JanitorCoroutines.CleanCoroutine(body));
                                break;
                            }
                        break;
                    ...
                    case CustomRPC.EraserLose:
                        Role.GetRole<Eraser>()?.Loses();
                        break;

                    case CustomRPC.EraserWin:
                        Role.GetRole<Eraser>()?.Wins();
                        break;
                }
            }
        }
        ...
        public static class RpcSetInfected
        {
            public static void Prefix([HarmonyArgument(0)] ref Il2CppReferenceArray<GameData.PlayerInfo> infected)
            {
                ...
                if (Check(CustomGameOptions.EraserOn))
                    NeutralRoles.Add((typeof(Eraser), CustomRPC.SetEraser, CustomGameOptions.EraserOn));
                #endregion
                ...
                if (Check(CustomGameOptions.SnitchOn))
                    CrewmateModifiers.Add((typeof(Snitch), CustomRPC.SetSnitch, CustomGameOptions.SnitchOn));
